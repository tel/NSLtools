function [sm, ALIGN, tt] = interpw1(ss, lambda)
% INTERPW1 1-D warping interpolation (128-point)
%	[sm, ALIGN, tt] = interpw1(ss, lambda);
...
for l = 1:L,
	r = lambda(l);
	s1 = interp1(t11, ss(:,1), (1-r)*t11+r*t21);
	s2 = interp1(t22, ss(:,2), (1-r)*t12+r*t22); 
	sm(:, l) =  ((1-r)*s1 + r*s2) * ((1-r)*g(1)+r*g(2));
	%sm(:, l) =  (r*s1 + (1-r)*s2) * ((1-r)*g(1)+r*g(2));
end;
toc;
